...
public class CartManager {
    ...
    public Cart getNewInsertedCartByLabel(String label) {
		Cart cart = new Cart();
		try {
			PreparedStatement preparedStatement = connection.prepareStatement("select * from carts where label=? and updated_at is null order by uid desc limit 1;");
    ...
    public boolean insertCart(Cart cart) {
    	...
    	try {
            PreparedStatement preparedStatement = connection
                    .prepareStatement("insert into carts( "
                            + "label, likes, type, price, "
                            + "price_save, ship_price, ship_code, status, "
            ...
            preparedStatement.setString(7, cart.getShipCode() != null && (!cart.getShipCode().isEmpty()) ? cart.getShipCode() :  "NONE");
    ...
    public void updateCart(Cart cart) {
        try {
            ...
            preparedStatement.setDouble(6, cart.getShipPrice());
            preparedStatement.setString(7, cart.getShipCode() != null && (!cart.getShipCode().isEmpty()) ? cart.getShipCode() :  "NONE");
            preparedStatement.setString(8, cart.getStatus() != null && (!cart.getStatus().isEmpty()) ? cart.getStatus() :  "DISABLE");
    ...
    public Cart getCart(int uid) {
        ...
        try {
            PreparedStatement preparedStatement = connection
                    .prepareStatement("select * from carts where uid=?");
